 ...
 *
 * @param peer the peer for which this is acting as a proxy
 * @param messageFilter a filter that process messages before they reach the peer it was sent to
 * @param user the user associated with this proxy's peer
 */
class PeerProxyActor(peer: PeerActor.Ref, messageFilter: PeerMessageFilterActor.Ref, user: PeerUser)
   ...
   *
   * The reason to kill the peer is to let it know about the ws disconnection event, it's not a perfect solution
   * but should fix the big problem related to phantom orders.
   */
